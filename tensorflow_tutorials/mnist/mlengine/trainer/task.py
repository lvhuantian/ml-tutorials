...
import math
import sys
from tensorflow_tutorials.mnist.mlengine.trainer.utils import maybe_download_and_ungzip
...
def conv_model(features, labels, mode, params):
    ...
    def batch_norm_cnv(inputs):
        return tf.layers.batch_normalization(inputs, axis=3, momentum=params['bnexp'], epsilon=1e-5, scale=False,
                                             training=(mode == tf.estimator.ModeKeys.TRAIN))

    def batch_norm(inputs):
        return tf.layers.batch_normalization(inputs, axis=1, momentum=params['bnexp'], epsilon=1e-5, scale=False,
                                             training=(mode == tf.estimator.ModeKeys.TRAIN))

    XX = tf.reshape(X, [-1, 28, 28, 1])
    Y1 = tf.layers.conv2d(XX, filters=params['conv1'], kernel_size=[6, 6], padding="same",
                          kernel_initializer=weights_init)
    Y1bn = tf.nn.relu(batch_norm_cnv(Y1))

    Y2 = tf.layers.conv2d(Y1bn, filters=params['conv2'], kernel_size=[5, 5], padding="same", strides=2,
                          kernel_initializer=weights_init)
    Y2bn = tf.nn.relu(batch_norm_cnv(Y2))

    Y3 = tf.layers.conv2d(Y2bn, filters=params['conv3'], kernel_size=[4, 4], padding="same", strides=2,
                          kernel_initializer=weights_init)
...
def main(argv):
    ...
    parser.add_argument('--job-dir', default="checkpoints",
    ...
    parser.add_argument('--hp-lr2', default=600, type=float,
                        help='Hyperparameter: learning rate decay speed in steps. Learning rate decays by exp(-1) every N steps.')
    ...
    parser.add_argument('--hp-bnexp', default=0.993, type=float,
                        help='Hyperparameter: exponential decay for batch norm moving averages.')
    parser.add_argument('--hp-iterations', default=3000, type=int,
