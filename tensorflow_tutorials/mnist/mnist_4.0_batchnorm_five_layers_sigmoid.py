...
# learning rate decay from 0.03 to 0.00001 speed 1000 => max 98.59 at 6500 iterations, 98.54 at 10K it, 98% at 1300it, 98.5% at 3200it
...
# accuracy of the trained model, between 0 (worst) adn 1 (best)
...
allbiases = tf.concat([tf.reshape(O1, [-1]), tf.reshape(O2, [-1]), tf.reshape(O3, [-1]),
                       tf.reshape(O4, [-1]), tf.reshape(B5, [-1])], 0)
# to use for sigmoid
...
# all biases are initalised at 0.1 apart from the last one which is initialised at 0.)

## decaying learning rate from 0.003 to 0.0001 decay_speed 2000, 10K iterations
# final test accuracy = 0.9818 (sigmoid - training cross-entropy not stabilised)
